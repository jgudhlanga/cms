...
{
    protected $signature = 'app:process-ojet-offer-letter-command';

    protected $description = 'Generate offer letters for OJET students';
                ...
                foreach ($programs as $program) {
                    ...
                    try {
                        DB::transaction(function () use ($program, $acceptedStep) {
                            ...
                            $this->info("Ojet offer letter generated successfully. {$program->id}");
                        });
                    } catch (Throwable $e) {
                        $this->error("Ojet offer letter failed for {$program->id}: {$e->getMessage()}");
                    }
                }
            });
    }
