...
class SponsorRepository extends BaseRepository implements ISponsorRepository
{
    ...
    {
        $sponsor = $this->sponsor->create($this->getFields($dto))->refresh();
        $this->saveContact($sponsor, $dto);
        $this->saveAddress($sponsor, $dto);
        return $sponsor;
    }

    public function update(Sponsor $sponsor, SponsorDto $dto)
    {
        $sponsor = tap($sponsor)->update($this->getFields($dto));
        $this->saveContact($sponsor, $dto);
        $this->saveAddress($sponsor, $dto);
        return $sponsor;
    }
    ...
    private function saveContact(Sponsor $sponsor, SponsorDto $dto): void
    {
        if ($this->hasContact($dto)) {
            ...
            # Update the first existing contact if available, otherwise create a new one
    ...
    private function saveAddress(Sponsor $sponsor, SponsorDto $dto): void
    {
        if ($this->hasAddress($dto)) {
            ...
            # Update the first existing address if available, otherwise create a new one
            $address = $sponsor->addresses->first();
            if ($address) {
    ...
    private function hasAddress(SponsorDto $dto): bool
    {
    ...
    private function hasContact(SponsorDto $dto): bool
    {
        return (bool)array_filter([
