...
class DepartmentLevelCourseResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'programmeStages' => $this->whenLoaded('programmeStages', fn () => $this->programmeStages->map(fn ($stage) => [
                'id' => $stage->id,
                'stageNumber' => $stage->stage_number,
                'code' => $stage->code,
                'name' => $stage->name,
                'kind' => $stage->kind?->value ?? $stage->kind,
            ])->values()),
            'programmeSemesters' => $this->whenLoaded('programmeSemesters', fn () => $this->programmeSemesters->map(fn ($semester) => [
                ...
                'programmeStageId' => $semester->programme_stage_id,
                'yearNumber' => $semester->year_number,
                'periodInYear' => $semester->period_in_year,
                'name' => $semester->name,
