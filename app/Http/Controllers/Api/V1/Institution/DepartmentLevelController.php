...
class DepartmentLevelController extends Controller
{
    ...
    public function enrolments(InstitutionDepartment $institutionDepartment, DepartmentLevel $departmentLevel)
    {
        ...
        $workflowSteps = WorkflowStepResource::collection(
            WorkflowHelper::getAllSteps()
        );
        ...
        $maxStep, $courseId)
    {
        $query = $institutionDepartment->studentApplications()
            ...
            ->whereHas('workflowStep', fn ($q) => $q->where('position', '<', $maxStep->position))
        ...
        return $query->get()
            ->groupBy(fn ($enrolment) => $enrolment->workflowStep->name)
            ->sortByDesc(fn ($group) => $group->first()->workflowStep->position ?? 0)
