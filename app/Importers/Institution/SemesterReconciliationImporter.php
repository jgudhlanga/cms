...
class SemesterReconciliationImporter
{
    /** @var list<string> */
    ...
    public const array HEADER_ALIASES = [
        ...
        'programme_phase' => ['PROGRAMME PHASE', 'PROGRAM PHASE', 'PHASE', 'SEMESTER', 'TERM', 'ABMA'],
    ];
    ...
    public function parse(string $filePath): array
    {
        ...
        if ($headerRowNumber === null) {
            throw new RuntimeException(__('trans.department_reconciliation_import_preview_failed'));
        }
        ...
        foreach (array_slice($sheetRows, $headerRowNumber) as $index => $row) {
            ...
            if ($this->isBlankRow($values) || $this->isSecondaryHeaderRow($values)) {
                continue;
            }
            ...
            $parsedRows[] = [
                ...
                'programmePhase' => $values['programme_phase'],
            ];
        }
     ...
     * @return array{student_number: int|null, level: int|null, course: int|null, programme_phase: int|null}
     */
    ...
    /**
     * @param  array<string, mixed>  $row
     * @param  array{student_number: int|null, level: int|null, course: int|null, programme_phase: int|null}  $columnMap
     * @return array{student_number: string|null, level: string|null, course: string|null, programme_phase: string|null}
     */
    private function extractRowValues(array $row, array $columnMap): array
    {
        ...
        return [
            ...
            'level' => $this->cellValue($values, $columnMap['level']),
            'course' => $this->cellValue($values, $columnMap['course']),
            'programme_phase' => $this->cellValue($values, $columnMap['programme_phase']),
        ];
    }
    ...
    /**
     * @param  array{student_number: string|null, level: string|null, course: string|null, programme_phase: string|null}  $values
     */
    ...
    private function isSecondaryHeaderRow(array $values): bool
    {
        ...
        $level = strtoupper((string) $values['level']);
        $course = strtoupper((string) $values['course']);
        $phase = strtoupper((string) $values['programme_phase']);

        return in_array($studentNumber, ['STUDENT NUMBER'], true)
            || in_array($level, ['LEVEL'], true)
            || in_array($course, ['COURSE', 'PROGRAMME', 'PROGRAM'], true)
            || in_array($phase, ['PROGRAMME PHASE', 'PROGRAM PHASE', 'PHASE', 'SEMESTER', 'TERM', 'ABMA'], true);
    }
