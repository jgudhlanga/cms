...
test('dashboard header uses live academic calendar context', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->has('academicCalendars', 2)
            ->where('academicCalendars.0.attributes.shortLabel', 'Sem 1 – 2026')
            ->where('academicCalendars.1.attributes.shortLabel', 'Sem 2 – 2026')
        );
});
...
test('dashboard lists all semesters for the resolved calendar year', function () {
    $this->travelTo('2026-05-15');
...
test('dashboard excludes semesters from other calendar years', function () {
    $this->travelTo('2026-09-15');
