...
it('exports finalised student enrolments to Student_Enrollment.csv', function (): void {
    ...
    expect($header)->toBe(StudentEnrollmentExportService::HEADERS)
        ...
        ->and($row[9])->toBe('12')
        ->and($row[10])->toBe($studentApplication->departmentCourse->course->name)
        ->and($row[11])->toBe('ICT101')
        ->and($row[12])->toBe('2025/2026')
        ->and($row[13])->toBe('Full Time')
        ->and($row[14])->toBe('2025/2026')
        ->and($row[15])->toBe('Semester 1')
        ->and($row[16])->toBe('No')
        ->and($row[17])->toBe('Yes')
        ->and($row[18])->toBe('ACME Bursary')
        ->and($row[19])->toBe('Parent')
        ->and($row[20])->toBe('Parent')
        ->and($row[21])->toBe('Jane Guardian')
        ->and($row[22])->toBe('jane@example.com')
        ->and($row[23])->toBe('0771234567')
        ->and($row[24])->toBe('45, Second Street, Bulawayo');
