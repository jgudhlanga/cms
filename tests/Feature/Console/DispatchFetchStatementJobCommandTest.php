...
it('reclaims stale processing windows then dispatches up to the limit', function () {
    $this->travelTo('2026-02-01 12:00:00');

    config()->set('custom.bank-statements.bank_statements_queue', 'bank-statements');
    config()->set('custom.bank-statements.processing_stale_minutes', 30);
    ...
    expect($jan1)->not->toBeNull()
        ->and($jan1->status)->toBe(ZBBankStatementFetchWindowStatus::Processing)
        ->and($jan1->attempt_count)->toBe(2)
        ->and($jan1->processing_started_at)->not->toBeNull();
...
it('does not dispatch pending windows whose window_end is after today', function () {
...
it('dispatches pending windows once window_end is on or before today', function () {
    $this->travelTo('2026-01-11 12:00:00');
