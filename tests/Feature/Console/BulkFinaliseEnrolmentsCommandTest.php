...
it('finalises verified students with matching payments in the date window', function () {
    ...
    $studentApplication = createVerifiedStudentApplication('STU001');
    $departmentStep = createEnrolledDepartmentStep($studentApplication);
    ...
    $freshStudentApplication = $studentApplication->fresh();
...
it('is idempotent when the bulk finalise command runs more than once for the same paid student', function () {
    ...
    $studentApplication = createVerifiedStudentApplication('STU001B');
    ...
    expect(StudentEnrolment::query()->where('student_id', $studentApplication->student_id)->count())->toBe(1);
});
...
it('fails the command when no academic calendar can be resolved for a paid student', function () {
    ...
    $studentApplication = createVerifiedStudentApplication('STU001C');
    createEnrolledDepartmentStep($studentApplication);
    ...
    expect(StudentEnrolment::query()->where('student_id', $studentApplication->student_id)->exists())->toBeFalse();
});
...
it('continues and records a failure when calendar type is missing for a paid student', function () {
    ...
    $missingCalendarTypeProgram = createVerifiedStudentApplication('STU001D');
    ...
    $validProgram = createVerifiedStudentApplication('STU001E');
    ...
    $validProgramClassList = ClassList::query()->where('student_application_id', $validProgram->id)->first();
...
it('keeps verified students unchanged when no matching payment exists', function () {
    ...
    $studentApplication = createVerifiedStudentApplication('STU002');

    $this->artisan('enrolments:bulk-finalise-enrolments-command')->assertSuccessful();

    expect($studentApplication->fresh()->program_status_id)->toBe(ClassListTypeEnum::VERIFIED->value);
});

it('applies explicit start-date and end-date options when filtering payments', function () {
    config()->set('custom.bank-statements.plan_anchor_start', '2026-01-01');

    $studentApplication = createVerifiedStudentApplication('STU003');
...
it('writes an XLSX failure report to local storage', function () {
    ...
    createVerifiedStudentApplication('STU004');
...
it('emails super users in production with the failure report', function () {
    ...
    createVerifiedStudentApplication('STU005');
