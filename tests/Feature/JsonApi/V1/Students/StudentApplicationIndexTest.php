...
function createStudentApplicationForJsonApiTest(): array
{
    ...
    return compact('student', 'studentApplication', 'department', 'course', 'workflowStep');
}

test('json api student programs index returns applications for student filter', function () {
    ['student' => $student, 'studentApplication' => $studentApplication, 'department' => $department, 'course' => $course, 'workflowStep' => $workflowStep] = createStudentApplicationForJsonApiTest();

    $tenant = Tenant::query()->firstOrFail();
    $user = User::factory()->create(['tenant_id' => $tenant->id]);
    $user->givePermissionTo(['view:student-applications', 'view:students']);
...
test('json api student programs index is forbidden without permissions', function () {
    ['student' => $student] = createStudentApplicationForJsonApiTest();
...
test('json api student programs index allows portal student for own records only', function () {
    ['student' => $student, 'studentApplication' => $studentApplication] = createStudentApplicationForJsonApiTest();

    $tenant = Tenant::query()->firstOrFail();
    $portalUser = User::factory()->create(['tenant_id' => $tenant->id]);
    $portalUser->givePermissionTo('manageOwnStudentApplicationDetails:students');
    ...
    $otherUser->givePermissionTo('manageOwnStudentApplicationDetails:students');
