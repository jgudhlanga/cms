...
<template>
	<Transition name="fade">
		<div v-if="isOpen(name)" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
			<!-- Modal Container -->
			<div :class="computedClass">
				<!-- Modal Header -->
				<div class="flex items-center justify-between px-6 pt-6">
					<h2 class="text-md font-semibold uppercase">{{ title }}</h2>
					<button class="rounded-full p-2 hover:bg-accent" @click="() => destroyModal()">
						<component :is="icons[IconName.close]" color="black" :size="26" />
					</button>
				</div>
				<div class="my-2 h-1 w-full border-b-[1px]"></div>
				<form :name="name" v-if="hasForm" @submit.prevent="() => (onFormAction ? onFormAction!() : null)"
				      class="flex flex-col">
					<!-- Modal Body -->
					<div class="space-y-6 px-6 py-4 flex-auto overflow-y-scroll">
						<slot name="body" />
					</div>
					<!-- Modal Footer -->
				...
				<div v-else class="flex flex-col">
					<div class="space-y-6 px-6 py-4 flex-auto overflow-y-auto relative">
						<slot />
					</div>
					<div class="mt-6 flex w-full justify-end space-x-3 border-t-[1px] px-6 py-5">
						<BaseButton type="button" :variant="ColorVariant.shade" @click="() => destroyModal()">
