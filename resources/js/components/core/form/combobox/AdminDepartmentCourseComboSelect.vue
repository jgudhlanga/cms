...
const { isLoading, levelCourses, listAdminLevelCourses } = useDepartmentLevels();
const { isItTrue } = useUtils();
onMounted(async () => {
    if (Number(props.departmentLevelId ?? '') > 0) {
        await listAdminLevelCourses(route('v1.dropdowns.department-level.courses', String(props.departmentLevelId)));
    }
});
...
watch(
    ...
    async (newValue) => {
        ...
        if (isItTrue(props.triggerSearch) && Number(newValue) > 0) {
            await listAdminLevelCourses(route('v1.dropdowns.department-level.courses', String(newValue)));
        }
    },
);
</script>
