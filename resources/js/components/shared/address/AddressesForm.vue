	...
	>
		<template #body>
			<div class="grid grid-cols-2 gap-3">
				<Address1
					v-model="form.address_1" @input="clearFormErrors(form, 'address_1')"
					:error="form.errors.address_1" />
				<Address2
					v-model="form.address_2" @input="clearFormErrors(form, 'address_2')"
					:error="form.errors.address_2" />
				<Address3
					v-model="form.address_3" @input="clearFormErrors(form, 'address_3')"
					:error="form.errors.address_3" />
				<Address4
					v-model="form.address_4" @input="clearFormErrors(form, 'address_4')"
					:error="form.errors.address_4" />
				<BaseCheckbox
					input-id="address_is_main"
					v-model="form.address_is_main"
					:label="$t('trans.main')" />
			</div>
		</template>
	</BaseModal>
