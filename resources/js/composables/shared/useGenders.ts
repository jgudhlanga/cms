            ...
            { header: trans_choice('trans.title', 1), accessorKey: 'attributes.title' },
            {
                ...
                cell: ({ row }: { row: { original: Gender } }) => {
                    ...
                    return moreActionButton(!!row.original?.attributes?.deletedAt, [
                        { key: 'edit', action: () => onOpenModal(can['update:genders'], row.original) },
                        {
                            key: 'archive',
                            action: () => onDelete(can['delete:genders'], route('genders.destroy', id), name),
                        },
                        {
                            key: 'restore',
                            action: () => onRestore(can['restore:genders'], route('genders.restore', id), name),
                        },
                        {
                            key: 'delete',
                            action: () => onForceDelete(can['forceDelete:genders'], route('genders.force-delete', id), name),
                        },
                    ]);
                },
            },
