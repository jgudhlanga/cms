...
const isModeLoaded = (modeId: string): boolean => {
    ...
    return Boolean(loadedModes.value[`${intakePeriod.value.value}:${modeId}`]);
};

const totalEnrolments = computed(() =>
    orderedModes.value.reduce((sum, mode) => sum + modeCount(mode), 0),
);
...
const syncUrlParams = () => {
    ...
    router.get(
        route('institution-departments.show', { department: institutionDepartmentId }),
        params,
...
const fetchModeTotals = async () => {
    ...
    try {
        const document = await HttpService.get(
            route('v1.department-metadata.enrolments', {
                institution_department: institutionDepartmentId,
                intake_period_id: intakePeriod.value.value,
            }),
...
const loadModePanel = async (modeId: string, force = false) => {
    if (!intakePeriod.value?.value || !modeId) {
        return;
    }

    const cacheKey = `${intakePeriod.value.value}:${modeId}`;
...
onMounted(async () => {
    ...
    const preferredMode =
        orderedModes.value.find((row) => String(row.id) === String(query.mode_of_study_id)) ??
...
<template>
    <div class="flex flex-col gap-3">
        ...
        <DataLoadingSpinner v-if="intakePeriodsLoading || modesOfStudyLoading || loadingMeta" />
