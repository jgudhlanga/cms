...
import { useEnrolmentVsClassListImport } from '@/composables/institution/useEnrolmentVsClassListImport';
...
const elevateButtonLabel = computed(() =>
...
const breadcrumbs: BreadcrumbItemInterface[] = [
    ...
    { transKey: 'trans.department_enrolment_vs_class_list' },
];
...
const handleElevate = (): void => {
    confirmElevate(selectedRows.value, clearSelection);
};
...
<template>
    <Head :title="$t('trans.department_enrolment_vs_class_list')" />

    <PageContainer :breadcrumbs="breadcrumbs" :back-url="backUrl">
        <template #backNavigationLeading>
            <div>
                <h2 class="text-lg font-semibold">{{ $t('trans.department_enrolment_vs_class_list') }}</h2>
        ...
        <div class="w-full min-w-0 space-y-4">
            <BaseAlert
                ...
                :description="$t('trans.department_enrolment_vs_class_list_page_description')"
            />
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div class="space-y-1">
                    <BaseInput
                        ...
                        input-id="enrolment-vs-class-list-calendar-year"
                        name="calendar_year"
                        type="number"
            ...
            >
                ...
                <div :key="fileFormKey" class="min-w-0 flex-1 space-y-2 md:max-w-xl">
                    <label class="text-xs font-bold uppercase text-muted-foreground" for="enrolment-vs-class-list-file">
                        {{ $t('trans.department_reconciliation_select_file') }}
                    </label>
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                        <input
                            id="enrolment-vs-class-list-file"
                            ref="fileInput"
                            type="file"
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="border-b bg-muted/30 text-left text-xs uppercase text-muted-foreground">
                                <th class="px-3 py-2">
                                    <BaseCheckbox
                                        input-id="enrolment-vs-class-list-select-all"
                                        label=""
                                ...
                                <th class="px-3 py-2">{{ $t('trans.department_reconciliation_column_system_department') }}</th>
                                ...
                                <th class="px-3 py-2">{{ $t('trans.department_reconciliation_column_workflow') }}</th>
                                <th class="px-3 py-2">{{ $t('trans.department_reconciliation_column_class_list') }}</th>
                        ...
                        <tbody>
                            <tr
                                ...
                                :class="[rowHighlightClass(row.status), { 'bg-muted/20': !row.isSelectable && row.status !== 'wrong_place' }]"
                            >
                                <td class="px-3 py-2">
                                    <BaseCheckbox
                                        :input-id="`enrolment-vs-class-list-row-${row.rowNumber}`"
                                        label=""
                                ...
                                <td class="px-3 py-2">{{ row.systemDepartment ?? '—' }}</td>
                                <td class="px-3 py-2">{{ row.systemLevel ?? '—' }}</td>
                                <td class="px-3 py-2">{{ row.systemCourse ?? '—' }}</td>
                                <td class="px-3 py-2">{{ row.workflowStep ?? '—' }}</td>
                                <td class="px-3 py-2">{{ row.classListType ?? '—' }}</td>
                                <td class="px-3 py-2">
