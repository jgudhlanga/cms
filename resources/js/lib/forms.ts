...
const onFormSuccess = (message: string, modalToClose?: string) => {
	if (modalToClose) {
...
function buildFormOptions(form: InertiaForm<any>, successMessage: string, errorMessage: string, modalToClose?: string) {
	return {
		onStart: () => toggleFormLoader(true),
		onFinish: () => onFormFinish(form),
		onSuccess: () => onFormSuccess(successMessage, modalToClose),
